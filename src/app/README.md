...
3. En **Authentication → Settings → Authorized domains**, comprueba que aparecen `pulso-training.web.app` y `pulso-training.firebaseapp.com`. No añadas ni intentes verificar `josema711.github.io`: el acceso con Google se realiza desde Firebase Hosting.
...
`josema711.github.io` pertenece a GitHub y no se puede verificar como dominio propio. Si Firebase muestra una pantalla de verificación para ese dominio, cancélala: no hace falta para esta configuración.
