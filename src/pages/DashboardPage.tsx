...
import { ArrowRight, Award, BarChart3, CalendarDays, Dumbbell, History, LibraryBig, Scale, Sparkles, TrendingUp } from 'lucide-react'
...
export function DashboardPage() {
  const data = useLiveQuery(async () => {
    const [workouts, activeWorkout, sets, measurements, records, allExercises, settings] = await Promise.all([db.workouts.where('status').equals('completed').reverse().sortBy('date'), db.workouts.where('status').equals('active').first(), db.sets.toArray(), db.bodyMeasurements.orderBy('date').reverse().toArray(), db.personalRecords.orderBy('achievedAt').reverse().limit(4).toArray(), db.exercises.toArray(), getSettings()])
    ...
    return { workouts, activeWorkout, measurements, records, exercises, weekCount: weekWorkouts.length, monthCount: monthWorkouts.length, weekVolume: calculateWorkoutVolume(weekSets), weekSets: effectiveSets(weekSets), recommendations }
  }, [])
  ...
  return <div className="page"><PageHeader eyebrow="RESUMEN" title="Tu entrenamiento, en contexto." subtitle="Continúa donde lo dejaste o revisa la información que te ayuda a progresar." />
    <div className="hero-grid"><Card className="hero-card"><div><span className="status-dot" /> {data.activeWorkout ? 'SESIÓN EN CURSO' : 'LISTO PARA ENTRENAR'}</div><h2>{data.activeWorkout ? (data.activeWorkout.name || data.activeWorkout.type) : data.workouts[0] ? `Última sesión: ${data.workouts[0].name || data.workouts[0].type}` : 'Tu primer registro empieza aquí'}</h2><p>{data.activeWorkout ? `Empezaste hoy a las ${data.activeWorkout.startTime}. Tus datos están guardados.` : data.workouts[0] ? `${data.workouts[0].date} · hace ${formatDistanceToNow(new Date(`${data.workouts[0].date}T${data.workouts[0].startTime}`), { locale: es })}` : 'Registra cada serie y Pulso conservará las cargas, repeticiones y sensaciones.'}</p><Link to="/entrenamiento"><Button className="hero-button"><Dumbbell /> {data.activeWorkout ? 'Continuar sesión' : data.workouts[0] ? 'Iniciar entrenamiento' : 'Registrar primera sesión'} <ArrowRight /></Button></Link></Card>
    ...
    <nav className="quick-actions" aria-label="Acciones rápidas">
      <Link to="/entrenamiento"><span><Dumbbell /></span><div><strong>{data.activeWorkout ? 'Continuar' : 'Entrenar'}</strong><small>{data.activeWorkout ? 'Retoma tu sesión activa' : 'Inicia una nueva sesión'}</small></div><ArrowRight /></Link>
      <Link to="/historial"><span><History /></span><div><strong>Historial</strong><small>Consulta sesiones anteriores</small></div><ArrowRight /></Link>
      <Link to="/ejercicios"><span><LibraryBig /></span><div><strong>Ejercicios</strong><small>Gestiona tu biblioteca</small></div><ArrowRight /></Link>
      <Link to="/progreso"><span><BarChart3 /></span><div><strong>Progreso</strong><small>Revisa tus tendencias</small></div><ArrowRight /></Link>
    </nav>
