...
import { formatSetPerformance } from '../logic/tracking'
...
export function WorkoutDetailPage() {
  ...
  return <div className="page"><Link to="/historial" className="back-link"><ArrowLeft /> Volver al historial</Link><PageHeader eyebrow={workout.status === 'completed' ? 'SESIÓN COMPLETADA' : 'BORRADOR'} title={workout.name || workout.type} subtitle={`${workout.date} · ${workout.startTime}${workout.endTime ? `–${workout.endTime}` : ''}`} action={<Button onClick={editWorkout}><Pencil /> {workout.status === 'active' ? 'Continuar editando' : 'Editar sesión'}</Button>} />
    <div className="stats-grid"><Stat label="Volumen de carga" value={`${formatNumber(calculateWorkoutVolume(sets), 0)} kg`} /><Stat label="Series completadas" value={effectiveSets(sets)} /><Stat label="Repeticiones" value={totalReps(sets)} /><Stat label="Ejercicios" value={bundle.exercises.length} /></div>
    ...
    <div className="detail-grid">{bundle.exercises.map((link) => { const exercise = exercises.find((e) => e.id === link.exerciseId); const exerciseSets = sets.filter((s) => s.workoutExerciseId === link.id).sort((a,b) => a.order-b.order); return <Card key={link.id}><h3>{exercise?.name || 'Ejercicio eliminado'}</h3><div className="detail-sets">{exerciseSets.map((set) => <div key={set.id}><span>{set.order + 1}</span><b>{exercise ? formatSetPerformance(set, exercise) : `${set.weight ?? '—'} kg × ${set.reps ?? '—'}`}</b><small>{set.type}{set.rir != null ? ` · RIR ${set.rir}` : ''}{set.rpe != null ? ` · RPE ${set.rpe}` : ''}</small><i className={set.completed ? 'yes' : ''}>{set.completed ? 'Hecha' : 'Pendiente'}</i></div>)}</div></Card> })}</div>
