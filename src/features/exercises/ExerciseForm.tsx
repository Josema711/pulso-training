...
import type { Equipment, Exercise, ExerciseType, ProgressionMethod } from '../../types'
...
const initial = { name: '', primaryMuscle: 'Pecho', type: 'Hipertrofia' as ExerciseType, equipment: 'Mancuernas' as Equipment, minIncrement: 2, targetMinReps: 8, targetMaxReps: 12, usualSets: 3, restSeconds: 90, progressionMethod: 'Doble progresión' as ProgressionMethod, technicalNotes: '', perDumbbell: true }
export function ExerciseForm({ exercise, onSave, onCancel }: { exercise?: Exercise; onSave: (value: Exercise) => Promise<void>; onCancel: () => void }) {
  const [form, setForm] = useState(exercise || initial); const [error, setError] = useState('')
  const set = (key: string, value: unknown) => setForm((old) => ({ ...old, [key]: value }))
  async function submit(event: FormEvent) { event.preventDefault(); const result = exerciseSchema.safeParse(form); if (!result.success) { setError(result.error.issues[0].message); return } const timestamp = nowIso(); await onSave({ secondaryMuscles: [], unit: 'Kilogramos', active: true, favorite: false, createdAt: timestamp, id: uid(), ...form, updatedAt: timestamp } as Exercise) }
  return <form onSubmit={submit} className="form-grid">
    ...
    <Input label="Incremento mínimo (kg)" type="number" step="0.25" min="0.01" value={form.minIncrement} onChange={(e) => set('minIncrement', Number(e.target.value))} />
    <div className="split"><Input label="Reps mín." type="number" min="0" value={form.targetMinReps} onChange={(e) => set('targetMinReps', Number(e.target.value))} /><Input label="Reps máx." type="number" min="0" value={form.targetMaxReps} onChange={(e) => set('targetMaxReps', Number(e.target.value))} /></div>
    ...
    <Select label="Método de progresión" value={form.progressionMethod} onChange={(e) => set('progressionMethod', e.target.value)}>{methods.map((item) => <option key={item}>{item}</option>)}</Select>
    ...
    {form.equipment === 'Mancuernas' && <label className="check full"><input type="checkbox" checked={form.perDumbbell} onChange={(e) => set('perDumbbell', e.target.checked)} /> El peso corresponde a cada mancuerna</label>}
